...
<div class="<?php echo $objectrow['class'];?>" id="<?php echo $objectrow['name'];?>" style=<?php if($objectrow['height']>0){echo "'height: ".$objectrow['height']."px;'";} else{echo "'height: 250px;'";}?>></div>
...
//Test connection and close/script and variable if failure occurs
try {
...
foreach($result as $key=>$value){
  ...
  if ($keyval==$column0){echo "{ x : ".$xval;} if($keyval==${'column'.$i}){ echo ", y : ".$val." },
";}
}}
echo "]},";
$i++;
...
var <?php echo $objectrow['name'];?> = nv.models.lineChart().margin({right: 35});/*fixes last pixel from getting clipped in bootstrap*/

<?php if($isdate==1){echo $objectrow['name'].".tooltip./*valueFormatter*/headerFormatter(function (d) {return d3.time.format('".$dateformat."')(new Date(d))});";}?>							
//Tooltip formatting
...
".$objectrow['name'].".lines.scatter.xScale(d3.time.scale());";
}
else {echo "
...
d3.select("#<?php echo $objectrow['name'];?>").append('svg')
.datum(<?php echo $objectrow['name'];?>Data())
.transition().duration(500).call(<?php echo $objectrow['name'];?>);
